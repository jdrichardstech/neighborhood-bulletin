...
class Comments extends Component {
  ...
  render() {
    ...
    const image =
      this.state.updated.commentImage == null
        ? ''
        : ImageHelper.thumbnail(this.state.updated.commentImage, 22);
    ...
    if (this.props.user != null) {
          ...
          commentList = zoneComments.map((comment, i) => {
            return (
              <div key={i}>
                <li>
                  <Comment
                    commentImage={this.state.updated.commentImage}
                    handleImage={this.uploadImage.bind(this)}
                    handleSubmitEdit={this.submitEdit.bind(this)}
      ...
      header = (
        <div>
            ...
            <div className="card-block">
              <div className="row">
                <div className="col-lg-12">
                  <h2>
                    Posts for{' '}
                    <span style={{ color: '#03a9f4' }}>{zoneName}</span>
                  </h2>
                  <center>
                    <img
                      style={{ width: '85%', maxHeight: 300 }}
                    ...
                    >
                      Create Post&nbsp;&nbsp;
                      <i className="zmdi zmdi-comment-outline" />
                    </button>
                  </center>
                </div>
              </div>
          ...
          <div>
            <ul style={{ listStyle: 'none', padding: 0 }}>{commentList}</ul>
          </div>
...
const dispatchToProps = dispatch => {
  return {
    ...
    updateComment: (comment, updated) =>
      dispatch(actions.updateComment(comment, updated)),
