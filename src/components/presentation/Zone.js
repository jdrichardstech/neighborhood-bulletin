...
class Zone extends Component {
  ...
  render() {
    ...
    const activeZone = this.props.isSelected ? (
      <a
        style={{ fontSize: '1.2em', color: '#ff9800' }}
    ...
    const button =
      this.props.currentZone.username == this.props.username ? (
        <Link to={'/updatezone/' + this.props.currentZone._id}>
          {' '}
          <button style={{ marginTop: 10 }} className="btn btn-warning">
            Update Zone
          </button>
        </Link>
      ) : null;
        ...
        <div className="media">
          ...
          <div className="media-body" style={{ padding: '20px 0 0 10px' }}>
            ...
            <div className="media-footer text-medium">
              <span className="mr-1">
                <i className="zmdi zmdi-city-alt color-default mr-05" />{' '}
                {zipCode}
              </span>
            </div>
          </div>
